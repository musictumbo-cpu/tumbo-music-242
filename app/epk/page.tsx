  ...
  return (
    <div className="pt-24 pb-16 max-w-5xl mx-auto px-4">
          ...
          {songs.map(song => (
            <div key={song.id} className="bg-dark-card border border-dark-border rounded-sm p-5">
              <div className="flex items-center gap-3">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full uppercase ${song.status === 'released' ? 'bg-gold/20 text-gold' : 'bg-gray-700 text-gray-400'}`}>
      ...
      <section className="mb-12">
        ...
        <p className="text-gray-300 text-sm">Email: {siteConfig.email}</p>
        <p className="text-gray-300 text-sm mt-1">Location: {siteConfig.location}</p>
      </section>s
    </div>
  )
}